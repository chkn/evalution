...
const BLOCK = id => `/* SPDX-License-Identifier: ${id} */\n/* ${COPYRIGHT} */\n`;
const XML = id => `<!-- SPDX-License-Identifier: ${id} -->\n<!-- ${COPYRIGHT} -->\n`;
...
function styleFor(file) {
  ...
  switch (name.slice(name.lastIndexOf('.') + 1)) {
    case 'ts': case 'tsx': case 'js': case 'mjs': case 'cjs': return LINE;
    case 'yml': case 'yaml': case 'toml': return HASH;
    case 'css': return BLOCK;
    case 'svg': case 'html': return XML;
    default: return null; // json, md, txt, LICENSE* → REUSE.toml
  }
}
...
function addHeader(file) {
  ...
  if (rel.startsWith('..') || rel.startsWith('node_modules/') || rel.startsWith('LICENSES/')) return 'covered';
  const style = styleFor(rel);
  if (!style) return 'covered';

  let body;
  try { body = readFileSync(abs, 'utf8'); } catch { return 'covered'; }
  if (body.split('\n').slice(0, 5).join('\n').includes(SPDX_TAG)) return 'skipped';

  const license = rel.startsWith('packages/vercel-ai-sdk/') ? 'MIT' : 'AGPL-3.0-only';
  const lines = body.split('\n');
  // The header must follow a shebang or HTML doctype if the file opens with one.
  const insertAt = lines[0]?.startsWith('#!') || /^<!doctype/i.test(lines[0] ?? '') ? 1 : 0;
  const before = lines.slice(0, insertAt).join('\n');
  const rest = lines.slice(insertAt).join('\n');
  writeFileSync(abs, (before ? before + '\n' : '') + style(license) + (rest.startsWith('\n') ? rest : '\n' + rest));
  return 'added';
}
...
const files = args.length ? args : execSync('git ls-files --cached --others --exclude-standard', { cwd: root, encoding: 'utf8' }).split('\n').filter(Boolean);
const tally = { added: 0, skipped: 0, covered: 0 };
for (const f of files) tally[addHeader(f)]++;
console.log(`Headers added: ${tally.added}, skipped (already tagged): ${tally.skipped}, not comment-capable: ${tally.covered}`);
