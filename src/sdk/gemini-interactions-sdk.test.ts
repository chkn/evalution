...
import type { ParsedPrompt, NormalizedPromptUpdates, PropValue } from '../shared/types.ts';
        ...
        if (name === 'generation_config' && props[name].kind === 'object') {
          return {
            ...
            type: {
              ...
              syntax: '{}',
              properties: Object.keys(props[name].kind === 'object' ? (props[name] as any).properties : {}).map(
                (subName: string) => ({ name: subName, type: { kind: 'primitive' as const, syntax: 'any' }, optional: true })
              ),
            },
            optional: true,
          };
        }
        return { name, type: { kind: 'primitive' as const, syntax: 'any' }, optional: true };
      }),
      values: props,
    },
  };
...
describe('GeminiInteractionsSDK', () => {
  ...
  describe('normalizePrompt', () => {
    ...
    it('exposes generation_config sub-properties as modelParameters', () => {
      ...
      expect(normalized.modelParameters[0].value).toEqual({ kind: 'primitive', value: 0.7 });
    ...
    it('preserves prompt metadata', () => {
      ...
      expect(normalized.metadata).toEqual({ relativeFilePath: 'prompts/test.prompt.ts' });
  ...
  describe('denormalizeUpdates', () => {
    it('maps model, system, messages to SDK property names', () => {
      ...
      expect(raw['model']).toEqual({ kind: 'primitive', value: 'gemini-2.5-pro' });
    ...
    it('removes generation_config when all params are deleted', () => {
      const currentValues = {
        generation_config: {
          kind: 'object' as const,
          properties: { temperature: { kind: 'primitive' as const, value: 0.7 } },
        },
      };
      const raw = sdk.denormalizeUpdates({ modelParameters: { temperature: null } }, currentValues);
  ...
  describe('file parsing integration', () => {
    it('normalizes system_instruction built from string concatenation as an editable primitive', async () => {
      ...
      const fixturePath = path.join(fixturesDir, 'gemini-concat-system.prompt.ts');
