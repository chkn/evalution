...
describe("MemoryFileProvider", () => {
  describe("readFile", () => {
    ...
    it("throws for an unknown path", async () => {
      ...
      await expect(provider.readFile("/virtual/missing.ts")).rejects.toThrow(
        "File not found",
      );
    });
  });
  ...
  describe("import", () => {
    ...
    it("throws for an unknown path", async () => {
      const provider = new MemoryFileProvider();
      await expect(provider.import("/virtual/missing.js")).rejects.toThrow(
        "File not found",
      );
    });
  });
  ...
  describe("glob", () => {
    ...
    it("returns absolute paths when absolute: true", async () => {
      ...
      const files = await Array.fromAsync(
        provider.glob("*.ts", { cwd, absolute: true }),
      );
  ...
  describe("watch", () => {
    ...
    it("only fires for files matching the include patterns", async () => {
      const provider = new MemoryFileProvider();
      const events: [string, string][] = [];
      provider.watch(["**/*.prompt.ts"], { cwd }, (type, fp) =>
        events.push([type, fp]),
      );
    ...
    it("excludes files matching ignored patterns", async () => {
      ...
      provider.watch(
        ["**/*.ts"],
        { cwd, ignored: ["node_modules/**"] },
        (type, fp) => events.push([type, fp]),
      );
    ...
    it("stops firing after cleanup is called", async () => {
      const provider = new MemoryFileProvider();
      const events: [string, string][] = [];
      const cleanup = provider.watch(["**/*.ts"], { cwd }, (type, fp) =>
        events.push([type, fp]),
      );
    ...
    it("supports multiple concurrent watchers", async () => {
      ...
      provider.watch(["**/*.ts"], { cwd }, (type, fp) =>
        events1.push([type, fp]),
      );
      provider.watch(["**/*.js"], { cwd }, (type, fp) =>
        events2.push([type, fp]),
      );
