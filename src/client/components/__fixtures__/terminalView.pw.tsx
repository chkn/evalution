...
function sentMessages(page: import('@playwright/test').Page): Promise<ClientMessage[]> {
...
test('starts the PTY at the rendered terminal size', async ({ mount, page }) => {
  const component = await mount(<TerminalViewHarness width={600} height={400} />);
...
test('reports a larger size when the container grows', async ({ mount, page }) => {
  const component = await mount(<TerminalViewHarness width={600} height={400} />);
