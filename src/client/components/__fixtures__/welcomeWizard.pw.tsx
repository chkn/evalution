...
const SETUP_TASKS = [
  {
    ...
    steps: [
      ...
      },
      {
        ...
        contents: "import { FilePromptProvider, VercelAISDK } from 'evalution';\n",
        completed: false,
      },
    ],
  },
...
test.beforeEach(async ({ page }) => {
  await page.route('**/api/setup-tasks', route => route.fulfill({ json: SETUP_TASKS }));
});
...
test('manual setup lists the SDK steps and links other SDKs externally', async ({ mount }) => {
  const component = await mount(<WelcomeWizardHarness />);
  ...
  await expect(component.getByRole('button', { name: /AI SDK/ })).toHaveAttribute('aria-pressed', 'true');
  const other = component.getByRole('link', { name: 'Other' });
  await expect(other).toHaveAttribute('target', '_blank');
});

test('an already-installed package shows as installed, not runnable', async ({ mount, page }) => {
  await page.route('**/api/setup-tasks', route => route.fulfill({
    json: [{
      ...SETUP_TASKS[0],
      steps: [{ ...SETUP_TASKS[0].steps[0], completed: true }, SETUP_TASKS[0].steps[1]],
    }],
  }));
  const component = await mount(<WelcomeWizardHarness />);

  // The install step row reports completion instead of offering a Run link.
  const installRow = component.locator('.setup-step', { hasText: '@evalution/vercel-ai-sdk' });
