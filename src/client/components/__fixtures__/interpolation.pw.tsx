...
test('typing ${ opens the dropdown with interpolatables + the literal action', async ({ mount, page }) => {
...
test('Enter accepts the suggestion and closes it into a token', async ({ mount, page }) => {
  await mockApiRoutes(page);
  const component = await mount(<InterpolationHarness />);
  const editor = component.locator('.token-editor').nth(1);
...
test('Tab accepts without closing; } then commits the token', async ({ mount, page }) => {
  await mockApiRoutes(page);
  const component = await mount(<InterpolationHarness />);
  const editor = component.locator('.token-editor').nth(1);
...
test('space commits the token right after accepting a suggestion', async ({ mount, page }) => {
  await mockApiRoutes(page);
  const component = await mount(<InterpolationHarness />);
  const editor = component.locator('.token-editor').nth(1);
...
test('space does NOT commit when the expression was typed freely', async ({ mount, page }) => {
  await mockApiRoutes(page);
  const component = await mount(<InterpolationHarness />);
  const editor = component.locator('.token-editor').nth(1);
...
test('typing } still creates a token after dismissing the dropdown with Escape', async ({ mount, page }) => {
  await mockApiRoutes(page);
  const component = await mount(<InterpolationHarness />);
  const editor = component.locator('.token-editor').nth(1);
...
test('drills into object children, then Enter commits the nested token', async ({ mount, page }) => {
  await mockApiRoutes(page);
  const component = await mount(<InterpolationHarness />);
  const editor = component.locator('.token-editor').nth(1);
...
test('without interpolatables, typing ${foo} still creates a token', async ({ mount, page }) => {
  await mockApiRoutes(page);
...
test('choosing "insert as literal text" keeps the text literal', async ({ mount, page }) => {
  await mockApiRoutes(page);
  const component = await mount(<InterpolationHarness withParams={false} />);
  const editor = component.locator('.token-editor').nth(1);
