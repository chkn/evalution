 ...
 * group, a resource, or one of its declared outputs.
 *
...
export function buildSourceTree(
  ...
  const outputsByParent = new Map<string, ResourceInfo[]>();
  for (const r of resources) {
    if (r.parent === undefined) continue;
    const list = outputsByParent.get(r.parent);
    if (list) list.push(r);
    else outputsByParent.set(r.parent, [r]);
  }

  const items: { group: readonly string[]; node: SourceNode }[] = [];
  for (const root of resources) {
    if (root.parent !== undefined) continue; // an output, handled via its root
    const node = resolveResource(
      root,
      outputsByParent.get(root.uri) ?? [],
      matched,
    );
...
function resolveResource(
  root: ResourceInfo,
  outputs: readonly ResourceInfo[],
  matched: ReadonlySet<string>,
): SourceNode | null {
  const group = root.group ?? [];
  const breadcrumb = breadcrumbOf(group, root.label);
  const selfSelectable = matched.has(root.uri);
  const matchedOutputs = outputs.filter(v => matched.has(v.uri));

  const outputNode = (v: ResourceInfo): SourceNode => ({
    kind: "output",
    uri: v.uri,
    label: v.label,
  ...
  if (!selfSelectable) {
    ...
    return {
      ...
      label: root.label,
      breadcrumb,
      children: matchedOutputs.map(outputNode),
    };
  }
  ...
  return {
    ...
    label: root.label,
    breadcrumb,
    children: [selfEntry, ...matchedOutputs.map(outputNode)],
  };
}
