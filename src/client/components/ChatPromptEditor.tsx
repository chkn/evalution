...
import { canEdit, isEditable } from "../../shared/helpers";
import type {
  ChatPromptUpdates,
...
export type ChatEditorUpdates = Omit<ChatPromptUpdates, "style">;

interface Props {
  prompt: NormalizedChatPrompt;
...
function MessageCard({
  ...
  messageIndex: number;
  /** Whether the message list can be edited at all (an SDK capability). */
  editable: boolean;
  propDef: PropDefinition;
  ...
  // Content that didn't parse into an editable shape (a call to a helper, an
  // arbitrary expression) is shown, not offered for editing: writing it back
  // would replace code the editor never understood.
  const contentEditable = canEdit(editable, msg.content);
    ...
    <div className="pg-panel-card" data-message-index={messageIndex}>
      ...
      {contentEditable ? (
        ...
        <div className="pg-msg-content" data-readonly="true">
          {valueToDisplayString(msg.content)}
        </div>
      )}
      {editable && msg.role === "assistant" && (
        <ToolCallsSection
...
/**
 * The editor for a {@link NormalizedChatPrompt}: model row, system message,
 * and a message list.
 */
export default function ChatPromptEditor({
  ...
  const handleAddMessage = () => {
    ...
    // Saving now, so drop any debounced save of the list before the addition.
    if (msgSaveTimer.current) clearTimeout(msgSaveTimer.current);
  ...
  const addableParams = modelDefinition
        ...
        <div className="pg-panel-model-row">
          ...
          <div className="pg-panel-model-row-actions">
            ...
            <button type="button" className="pg-pill-btn">
              Output type: Text
              <span aria-hidden className="pg-pill-chevron">
                ▾
              </span>
            </button>
          </div>
      ...
      <div className="pg-panel">
        ...
        >
          <MemoSystemCard
            ...
            editable={canEdit(prompt.systemEditable, prompt.system)}
          ...
          {localMessages.map((msg, i) => (
            <MessageCard
              ...
              msg={msg}
              messageIndex={i}
              editable={prompt.messagesEditable}
              propDef={contentPropDef}
              onChange={m =>
        ...
        <div className="pg-panel-footer">
          <button
            ...
            disabled={!prompt.messagesEditable}
          >
            ＋ Add message
          </button>
        </div>
