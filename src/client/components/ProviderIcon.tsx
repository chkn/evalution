...
function RawProviderIcon({ icon, size }: { icon: string; size: number }) {
  return (
    <span
      style={{ display: 'inline-flex', width: size, height: size, fontSize: size }}
...
function FallbackIcon({ provider, size }: { provider?: string; size: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 24 24">
      ...
      <text x="12" y="16.5" textAnchor="middle" fill="white" fontSize="13" fontWeight="600" fontFamily="system-ui, sans-serif">{letter}</text>
    </svg>
  );
}
