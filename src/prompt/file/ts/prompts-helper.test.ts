...
import { LocalFileProvider, MemoryFileProvider } from '../../../file-provider.ts';
...
describe('TSPromptFileType — prompts() helper', () => {
  describe('parsing', () => {
    ...
    it('extracts model, system, messages, and other props', async () => {
      ...
      const weather = parsed.find(p => p.name === 'checkWeather')!;
      expect(getValue(weather, 'system')).toEqual({ kind: 'primitive', value: 'You are a weather assistant' });
      expect(getValue(weather, 'temperature')).toEqual({ kind: 'primitive', value: 0.7 });
      expect(getValue(weather, 'maxTokens')).toEqual({ kind: 'primitive', value: 500 });
...
`;
    ...
    it('updates a property of a helper-defined prompt', async () => {
      ...
      await ft.updateProperty(filePath, getDef(parsed[0], 'system'), { kind: 'primitive', value: 'New system' });
    ...
    it('adds a property to a helper-defined prompt', async () => {
      ...
      await ft.addProperty(filePath, 'myPrompt', 'maxTokens', { kind: 'primitive', value: 1000 });
...
`;
    ...
    it('augments the destructure when switching to a new provider rather than adding a top-level import', async () => {
      ...
      expect(after).not.toContain("import { anthropic } from '@ai-sdk/anthropic'");
    });
...
`;
    ...
    it('invokes a helper-defined prompt and returns its config', async () => {
      ...
      expect(config).toEqual({ model: 'openai/gpt-4o', system: 'hi world', messages: [] });
    });
  });
});
