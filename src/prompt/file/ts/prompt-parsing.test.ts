...
function getDef(prompt: { extractedProps: { definitions: { name: string; valueSpan?: { start: number; end: number } }[] } }, name: string) {
...
describe('PromptParser', () => {
  describe('basic parsing', () => {
    ...
    it('should parse file with multiple exported functions', async () => {
      const { paths, fileType } = await loadFixtures('multiple-exports.prompt.ts');
      const prompts = await fileType.parsePrompts([paths[0]], '');
  ...
  describe('property parsing', () => {
    it('should parse string literal parameters as PropValue', async () => {
      ...
      expect(system).toEqual({ kind: 'primitive', value: 'You are a weather assistant' });
  ...
  describe('template values', () => {
    ...
    it('should handle destructured object parameters', async () => {
      const { paths, fileType } = await loadFixtures('destructured-param.prompt.ts');
      const prompts = await fileType.parsePrompts([paths[0]], '');
    ...
    it('should handle destructured parameters with default values', async () => {
      const { paths, fileType } = await loadFixtures('destructured-defaults.prompt.ts');
      const prompts = await fileType.parsePrompts([paths[0]], '');
  ...
  describe('model parameter', () => {
    ...
    it('should parse model parameter - function call format', async () => {
      const { paths, fileType } = await loadFixtures('function-model.prompt.ts');
      const prompts = await fileType.parsePrompts([paths[0]], '');
  ...
  describe('source information', () => {
    it('should extract source spans for all parameters', async () => {
      ...
      expect(systemDef.valueSpan!.end).toBeGreaterThan(systemDef.valueSpan!.start as number);
    });
  });
  ...
  describe('editability', () => {
    ...
    it('should handle dynamic/computed values (mark as read-only)', async () => {
      const { paths, fileType } = await loadFixtures('dynamic-values.prompt.ts');
      const prompts = await fileType.parsePrompts([paths[0]], '');
  ...
  describe('edge cases', () => {
    it('should handle files with syntax errors gracefully', async () => {
      const { paths, fileType } = await loadFixtures('invalid-syntax.prompt.ts');
      const prompts = await fileType.parsePrompts([paths[0]], '');
  ...
  describe('parseAll', () => {
    it('should parse all files in the program', async () => {
      const { paths, fileType } = await loadFixtures('basic.prompt.ts', 'function-model.prompt.ts');
