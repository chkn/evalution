...
function stubProvider(
  id: string,
  prompts: Partial<NormalizedPrompt>[],
): PromptProvider {
  return {
    id,
    async getAllPrompts() {
...
function registryWith(providers: PromptProvider[]): Promise<PromptRegistry> {
  ...
  return registry
    .rebuild(new Map(providers.map(p => [p.id, p])))
    .then(() => registry);
}
...
describe("PromptRegistry", () => {
  ...
  it("reflects the latest prompts after a rebuild", async () => {
    const registry = new PromptRegistry();
    const provider = stubProvider("fs", [
      { id: "a.prompt.ts#foo", globalId: "mod#foo" },
    ]);
    ...
    const moved = stubProvider("fs", [
      { id: "b.prompt.ts#foo", globalId: "mod#foo" },
    ]);
