...
function makeTracer(ingestor: OTelTraceIngestor) {
...
describe("OTelTraceIngestor", () => {
  ...
  it("stores the prompt reference raw — global id without a provider, scoped id with one", async () => {
    ...
    expect(scopedLoaded?.spans[0].prompt).toEqual({
      id: "a.prompt.ts#summarize",
      providerId: "fs",
    });
  });
  ...
  it("keeps attributes set at creation as well as ones set after the span starts", async () => {
    ...
    await ingestor.drainPendingHandlers();
    const loaded = await provider.getTrace(traceId);
    expect(loaded?.spans[0].attributes).toMatchObject({
      "at.start": "a",
      "after.start": "b",
    });
  });
  ...
  it("reports another OTelTraceIngestor as redundant", () => {
    const a = new OTelTraceIngestor();
    const b = new OTelTraceIngestor();
    expect(a.isRedundant(b)).toBe(true);
  });
});
